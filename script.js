...
function scrollToBottom(element) {
    ...
    // استخدام setTimeout لضمان تنفيذ التمرير بعد اكتمال عملية الرسم
    setTimeout(() => {
        // ضبط قيمة scrollTop لضمان التمرير للأسفل بالكامل
        ...
        if (settings && settings.animations !== false) {
            // تمرير سلس باستخدام الأنيميشن
            element.style.scrollBehavior = 'smooth';
            element.scrollTop = maxScrollTop + 50; // إضافة قيمة إضافية لضمان التمرير للأسفل بالكامل
            
            // إعادة تعيين سلوك التمرير الافتراضي بعد الانتهاء
            setTimeout(() => {
                element.style.scrollBehavior = 'auto';
            }, 500);
        } else {
            // تمرير فوري بدون أنيميشن
            element.scrollTop = maxScrollTop + 50;
        }
        
        // تأكيد إضافي للتمرير الكامل (للتعامل مع أي تغييرات لاحقة في المحتوى)
        setTimeout(() => {
    ...
    // إضافة تحسين للتمرير على الأجهزة المحمولة لضمان رؤية منطقة الكتابة
    if (window.innerWidth <= 768) {
        // تأخير قصير للتأكد من تطبيق جميع التغييرات
        setTimeout(() => {
            // التمرير بشكل مستقل عن وضع لوحة المفاتيح
            const inputContainer = document.querySelector('.chat-input-container');
            if (inputContainer) {
                // استخدام العنصر الأخير في المحادثة كنقطة مرجعية
                const lastMessage = element.querySelector('.message:last-child');
                if (lastMessage) {
                    // التمرير مع مراعاة ارتفاع منطقة الكتابة
                    lastMessage.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
                    
                    // تمرير إضافي لضمان وضوح الرسالة بشكل كامل
                    setTimeout(() => {
