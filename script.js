...
async function analyzeFile(fileData, mimeType, fileName, text) {
    try {
        console.log(`Starting analysis for ${fileName} with MIME type: ${mimeType || 'unknown'}`);
        
        // تحقق إذا كان fileData هو مسار URL أو بيانات Base64
        let base64Data;
        let finalMimeType = mimeType;
        
        if (fileData && fileData.startsWith('data:')) {
            // إذا كان بيانات Base64، استخرج القسم المطلوب
            const dataParts = fileData.split(',');
            base64Data = dataParts[1];
            
            // إذا لم يتم توفير mimeType، استخرجه من data URL
            if (!finalMimeType) {
                const mimeMatch = dataParts[0].match(/data:([^;]+)/);
                if (mimeMatch && mimeMatch[1]) {
                    finalMimeType = mimeMatch[1];
                }
            }
            
            console.log(`Extracted base64 data (length: ${base64Data ? base64Data.length : 0})`);
            console.log(`Using MIME type: ${finalMimeType || 'unknown'}`);
        } else {
            // مسار URL لا يمكن تحليله مباشرة، نحتاج إلى إخطار المستخدم
            console.error('File data is invalid or not in the expected format:', 
                          fileData ? fileData.substring(0, 50) + '...' : 'none');
            showNotification('خطأ في تنسيق بيانات الملف', 'error');
            return 'لا يمكن تحليل هذا النوع من الملفات. يرجى المحاولة مرة أخرى.';
        }

        if (!base64Data) {
            console.error('No base64 data extracted from file data');
            showNotification('خطأ في معالجة بيانات الملف', 'error');
            return 'حدث خطأ أثناء قراءة بيانات الملف. يرجى المحاولة مرة أخرى.';
        }

        if (!finalMimeType) {
            // افتراضي لنوع MIME الصورة إذا لم يتم تحديده
            finalMimeType = 'image/jpeg';
            console.log('No MIME type detected. Using default:', finalMimeType);
        }
        ...
        console.log(`Sending analysis request to API with MIME type: ${finalMimeType}`);
...
// دالة جديدة لإنشاء رابط ملف بدلاً من عرض الصورة
function createFileLink(fileUrl, mimeType) {
    ...
    let fileType = "صورة";
    if (mimeType) {
        fileType = mimeType.split('/')[1] || "صورة";
        ...
        if (mimeMatch && mimeMatch[2]) {
            fileType = mimeMatch[2];
        }
    }
    
    fileLink.innerHTML = `📷 صورة مرفقة (${fileType})`;
    fileLink.target = '_blank';
    fileLink.download = `image.${fileType}`;
    
    // تنسيق الرابط
    fileLink.style.display = 'block';
    fileLink.style.margin = '10px 0';
    fileLink.style.padding = '10px 15px';
    fileLink.style.backgroundColor = 'var(--border-color)';
...
async function addMessage(text, fileUrl = null, save = true, isAI = false, chatMessages, showThinking = true, fileMetadata = null) {
    console.log('Adding message:', { text, fileUrl: fileUrl ? 'file present' : 'no file', isAI, fileMetadata });
    ...
    message.className = `message ${isAI ? 'received' : 'sent'}`;
    
    const avatar = document.createElement('div');
    avatar.className = 'message-avatar';
    
    const avatarIcon = document.createElement('i');
    avatarIcon.className = isAI ? 'fas fa-robot' : 'fas fa-user';
    ...
    sender.textContent = isAI ? 'NeuroX' : 'أنت';
    ...
    // إذا كان هناك ملف صورة، إنشاء رابط بدلاً من عرض الصورة
    if (fileUrl) {
        ...
        if (fileUrl.startsWith('data:')) {
            mimeType = fileUrl.split(';')[0].split(':')[1];
        }
        
        // استخدام fileMetadata إذا كان موجودًا
        if (fileMetadata && fileMetadata.mimeType) {
            mimeType = fileMetadata.mimeType;
        }
        
        // التحقق مما إذا كان ملف صورة
        if (fileUrl.match(/\.(jpeg|jpg|gif|png|webp|svg|bmp)$/i) || fileUrl.startsWith('data:image')) {
            // استخدام دالة createFileLink
            const fileLink = createFileLink(fileUrl, mimeType);
            ...
            textDiv.innerHTML = `<a href="${fileUrl}" download>تحميل الملف (${mimeType ? mimeType.split('/')[1] : 'ملف'})</a>`;
    ...
    if (text) {
        ...
        if (isAI) {
            msgContent.appendChild(contentContainer);
        ...
        // إذا لم يكن هناك نص، أضف المحتوى مباشرة
        msgContent.appendChild(contentContainer);
        message.appendChild(msgContent);
        chatMessages.appendChild(message);
    }

    // تحليل الملف إذا كان موجوداً
    if (fileUrl) { // تم تعديل الشرط للسماح بتحليل الملف حتى مع وجود نص
        ...
        let fileName = fileUrl.split('/').pop() || 'ملف مرفق';
        let fileMimeType = '';
        
        // استخدام معلومات الملف من fileMetadata إن وجدت
        if (fileMetadata && fileMetadata.mimeType) {
            fileMimeType = fileMetadata.mimeType;
            console.log(`Using MIME type from metadata: ${fileMimeType}`);
        } else if (fileUrl && fileUrl.startsWith('data:')) {
            // استخراج نوع MIME من data URL
            const mimeMatch = fileUrl.match(/data:([^;]+)/);
            if (mimeMatch && mimeMatch[1]) {
                fileMimeType = mimeMatch[1];
                console.log(`Extracted MIME type from URL: ${fileMimeType}`);
            }
        }
        
        // إذا لم يتم العثور على نوع MIME، حاول تخمينه
        if (!fileMimeType && fileUrl) {
            if (fileUrl.match(/\.(jpeg|jpg|png)$/i)) {
                const ext = fileUrl.match(/\.([^.]+)$/)[1].toLowerCase();
                fileMimeType = `image/${ext === 'jpg' ? 'jpeg' : ext}`;
                console.log(`Guessed MIME type from extension: ${fileMimeType}`);
            } else {
                // نوع افتراضي للصور إذا لم نتمكن من تحديد النوع
                fileMimeType = 'image/jpeg';
                console.log(`Using default MIME type: ${fileMimeType}`);
            }
        }
        ...
        // تأكد من أن fileUrl ليس undefined أو null
        if (!fileUrl) {
            console.error('File URL is missing!');
            showNotification('خطأ: بيانات الملف مفقودة', 'error');
            chatMessages.removeChild(thinkingDiv);
            return;
        }
        
        const analysis = await analyzeFile(fileUrl, fileMimeType, fileName, text);
        chatMessages.removeChild(thinkingDiv);
...
// دالة لإنشاء عارض الصور - معطلة حالياً
function createImageViewer(imageUrl) {
    // تم تعطيل هذه الوظيفة حسب طلب المستخدم
    console.log("وظيفة عرض الصور معطلة", imageUrl);
    return null;
}
    ...
    if (messageForm) {
        messageForm.addEventListener('submit', async (e) => {
            ...
            if (message || currentFileData) {
                ...
                // حفظ بيانات الملف ونوع MIME قبل حذفهما من واجهة المستخدم
                const tempFileData = currentFileData;
                const tempMimeType = currentMimeType;
                
                console.log('Submitting message with file:', { 
                    hasFile: !!tempFileData, 
                    mimeType: tempMimeType,
                    fileLength: tempFileData ? tempFileData.length : 0,
                    messageText: message ? message.substring(0, 30) + '...' : 'No text'
                });
                ...
                // الآن نضيف الرسالة مع البيانات المؤقتة للملف ونوع MIME
                // إرسال بيانات إضافية لـ addMessage - نوع MIME
                const fileMetadata = tempFileData ? { mimeType: tempMimeType || (tempFileData.startsWith('data:') ? tempFileData.split(';')[0].split(':')[1] : 'image/jpeg') } : null;
                await addMessage(message, tempFileData, true, false, chatMessages, true, fileMetadata);
        ...
        fileUpload.addEventListener('change', (e) => {
            ...
            if (file) {
                ...
                reader.onload = (e) => {
                    ...
                    console.log('File loaded:', { type: file.type, name: file.name, size: file.size });
                    
                    filePreview.innerHTML = '';
                    
                    // تغيير طريقة عرض الصور لعرض نص بدلاً من الصورة
                    if (file.type.startsWith('image/')) {
                        const fileTypeText = document.createElement('div');
                        fileTypeText.className = 'file-type-text';
                        fileTypeText.innerHTML = `<i class="fas fa-image"></i> صورة مرفقة (${file.type.split('/')[1]})`;
                        fileTypeText.style.padding = '10px';
                        fileTypeText.style.backgroundColor = 'var(--border-color)';
                        fileTypeText.style.borderRadius = '8px';
                        fileTypeText.style.marginTop = '5px';
                        ...
                        const pdfDiv = document.createElement('div');
                        pdfDiv.className = 'pdf-preview';
                        pdfDiv.innerHTML = '<i class="fas fa-file-pdf"></i> ملف PDF';
                        filePreview.appendChild(pdfDiv);
                    } else {
                        const textDiv = document.createElement('div');
                        textDiv.className = 'text-file-preview';
                        textDiv.innerHTML = `<i class="fas fa-file-alt"></i> ملف ${file.type.split('/')[1] || 'نصي'}`;
